...
const TYPOGRAPHY_PAIRS: Record<number, { name: string; heading_font: string; body_font: string; google_fonts: string }> = {
  ...
  18: { name: "Recoleta-style (Fraunces) + Inter", heading_font: "Fraunces", body_font: "Inter", google_fonts: "Fraunces:ital,opsz,wght@0,9..144,100..900;1,9..144,100..900&family=Inter:wght@300;400;500;600" },
...
Deno.serve(async (req) => {
  try {
    ...
    const archetype = business.personality_profile.design_archetype || "Warm Local";
    ...
    const baseSections = ["About", "Services", "Reviews", "Hours", "Contact"];
    const sectionOrder = [...baseSections].sort(() => Math.random() - 0.5);
    const fingerprint = `${archetype}-${paletteId}-${typographyId}-${layout}`;

    if (existingFingerprints.includes(fingerprint)) {
      paletteId = pickRandom(validPalettes.filter(p => p !== paletteId));
      typographyId = pickRandom(validTypography.filter(t => t !== typographyId));
    }
    ...
    const reviewsText = (business.top_reviews || [])
      .map((r: { author: string; text: string; rating: number }) => `"${r.text}" — ${r.author}`)
      .join("\n");
...
COPYWRITING RULES:
- Hero headline: 4-8 words MAX, specific, NEVER "Welcome to" or "Quality you can trust"
- Subhead: one sentence referencing city/neighborhood
- About: first person if sole-proprietor, under 120 words, references something real from reviews, NEVER starts with "Welcome" or "We are"
- Services: 4-6, human descriptions
- Social proof: 2-3 verbatim review quotes with author names

BANNED PHRASES: "We pride ourselves", "Excellence in every", "Your trusted partner", "leverage", "solutions", "premier", "state-of-the-art", "Welcome to [Name]", "Quality you can trust", "Serving the community for", "At [Business Name], we believe", "Look no further"

DESIGN: Use provided palette exactly, mobile-first, smooth animations, cubic-bezier(0.4, 0, 0.2, 1)
FOOTER: "Built as a free preview — not affiliated with ${business.name}"
OUTPUT: Single HTML file with embedded <style> and <script>. No external CSS frameworks except Google Fonts. No placeholder content.`;
...
Micro-interactions to implement: ${microInteractions.join(", ")}
