...
Deno.serve(async (req) => {
  try {
    ...
    const businesses = await dbFilter('Business', { id: business_id }) as Record<string, unknown>[];
    if (!businesses?.length) return Response.json({ error: 'Business not found' }, { status: 404 });
    const business = businesses[0];

    const sites = await dbFilter('GeneratedSite', { business_id }) as Record<string, unknown>[];
    const site = sites?.[0] || {};
...
Banned phrases: "hope this finds you", "wanted to reach out", "amazing", "incredible", "premier".`;
